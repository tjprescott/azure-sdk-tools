...
function buildParameters(variables: Variables): Record<string, { value: unknown }> {
    return {
        ...
        entraAppId: { value: variables.entraAppId },
        entraAppIdUrl: { value: variables.entraAppIdUrl },
...
function siteConfig(environmentName: string): Record<string, unknown> {
    return {
        ...
        appSettings: [
            ...
            { name: "ENTRA_APP_ID", value: "[parameters('entraAppId')]" },
            { name: "ENTRA_APP_ID_URL", value: "[parameters('entraAppIdUrl')]" },
            { name: "ENVIRONMENT_NAME", value: environmentName },
            { name: "SCM_DO_BUILD_DURING_DEPLOYMENT", value: "true" },
            { name: "TENANT_ID", value: "[parameters('tenantId')]" },
            { name: "WEBAPP_ENDPOINT", value: "[format('https://{0}.azurewebsites.net/', parameters('webAppName'))]" },
            { name: "WEBAPP_NAME", value: "[parameters('webAppName')]" },
